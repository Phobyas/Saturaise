PredictiveSearch = {
  init: function init(context) {
    ...
    // Get DOM elements
    context.input = context.querySelector(context.inputSelector)
      ? context.querySelector(context.inputSelector)
      : context.querySelector(selectors.input);
    context.predictiveSearchResults = context.querySelector(
      this.resultsSelector
    )
      ? context.querySelector(context.resultsSelector)
      : context.querySelector(selectors.results);
  ...
  featureDetections: function featureDetections() {
    if (!this.isFunction(WAU.Helpers.debounce)) {
      console.log(
        "Error. Missing debouce theme helper method for predictive search."
      );
      return false;
    }
    if (!window.fetch) {
      console.log(
        "Error. Web browser does not support fetch required for predictive search"
      );
      return false;
    }
  },
  ...
  getSearchResults: function getSearchResults(searchTerm) {
    ...
    fetch(
      `${context.routesPredictiveSearchURL}?q=${encodeURIComponent(
        searchTerm
      )}&section_id=predictive-search&resources[limit]=9&resources[limit_scope]=each`
    )
      .then((response) => {
        if (!response.ok) {
...
document
  .querySelectorAll('[data-section-type="predictive-search"]')
